...
sum(predicciones==pre) #655, 656, 655
sum(predicciones==pre2) #658, 659, 658
sum(predicciones==pre3) #683, 682, 681
sum(predicciones==pre4) # 681
sum(predicciones==pre5)
sum(predicciones==pre6)
sum(predicciones==pre7)
...
# Quito a train la clase y variables malas
train <- train[,-c(var[1:25],75)]
...
library(caret)
modeloCaret0 <- caret::train(training0[,-50], as.factor(training0[,50]), method="glm", family=binomial(),
                             preProcess = c("center", "scale"),
                             tuneLength = 10,
                             trControl = trainControl(method = "cv"))
modelo.predict0 <- caret::predict.train(modeloCaret0, Test[,-75], type="prob")
modeloCaret1 <- caret::train(training1[,-50], as.factor(training1[,50]), method="glm", family=binomial(),
                             preProcess = c("center", "scale"),
                             tuneLength = 10,
                             trControl = trainControl(method = "cv"))
modelo.predict1 <- caret::predict.train(modeloCaret1, Test[,-75], type="prob")
modeloCaret2 <- caret::train(training2[,-50], as.factor(training2[,50]), method="glm", family=binomial(),
                             preProcess = c("center", "scale"),
                             tuneLength = 10,
                             trControl = trainControl(method = "cv"))
modelo.predict2 <- caret::predict.train(modeloCaret2, Test[,-75], type="prob")
modeloCaret3 <- caret::train(training3[,-50], as.factor(training3[,50]), method="glm", family=binomial(),
                             preProcess = c("center", "scale"),
                             tuneLength = 10,
                             trControl = trainControl(method = "cv"))
modelo.predict3 <- caret::predict.train(modeloCaret3, Test[,-75], type="prob")
...
write.table(predicciones, "sample.csv", col.names = TRUE, row.names = FALSE,
            quote=FALSE, sep = ",")
# 0.67153 en test, 0.65206 IPF+Smote, 0.62773 TomekLinks+Smote (200/100),0.55961 IPF-ROS, 0.56934 over (propia), 0.56934IPF-over propia